...
exports.run = async (client, message, args) => {
  ...
  if (tocheck === false) {
    return message.channel
      .send(
        '**⚠️ Veuillez demander à un administrateur de setup les roles modérateurs avec la commande `setup banrole [role]`⚠️** '
      )
      .then((msg) => {
        msg.delete({
          timeout: 5000
  ...
  if (message.member.roles.cache.get(tocheck)) {
    ...
    if (!args[0] || !args[1]) {
      return message.channel
        .send('**⚠️ Vérifier votre commande `banrole [membre] [raison]` ⚠️** ')
        .then((msg) => {
          msg.delete({
            timeout: 5000
          })
